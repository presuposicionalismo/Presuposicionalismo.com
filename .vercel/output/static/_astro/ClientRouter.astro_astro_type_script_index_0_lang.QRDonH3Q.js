...
function $(e) {
  ...
  return H.has(t) ? !0 : (H.add(t), !1);
}
function oe(e) {
  for (const t of e.scripts)
    !t.hasAttribute("data-astro-rerun") && $(t) && (t.dataset.astroExec = "");
}
function re(e) {
  const t = document.documentElement,
    n = [...t.attributes].filter(
      ({ name: o }) => (t.removeAttribute(o), ne.includes(o)),
    );
...
function W(e) {
  e.querySelectorAll("template[shadowrootmode]").forEach((t) => {
    ...
    if ((n === "closed" || n === "open") && o instanceof HTMLElement) {
      ...
      r.appendChild(t.content), t.remove(), W(r);
    }
  });
}
const ae = () => {
  ...
  pe = "astro:after-swap",
  we = "astro:page-load",
  D = (e) => document.dispatchEvent(new Event(e)),
  V = () => D(we);
...
class ge extends j {
  ...
  loader;
  constructor(t, n, o, r, i, u, a, c, d, l) {
    super(fe, { cancelable: !0 }, t, n, o, r, i, u, a, c),
      (this.formData = d),
...
async function be(e, t, n, o, r, i, u, a, c) {
  ...
  return (
    document.dispatchEvent(d) &&
      (await d.loader(),
      d.defaultPrevented ||
        (D(me), d.navigationType !== "traverse" && I({ scrollX, scrollY }))),
    d
  );
}
const I = (e) => {
...
const x = !!document.startViewTransition,
  N = () => !!document.querySelector('[name="astro-view-transitions-enabled"]'),
  K = (e, t) => e.pathname === t.pathname && e.search === t.search;
let m, g, S;
const Te = () => {
...
async function Ae(e, t) {
  try {
    ...
    for (const [a, c] of Object.entries(U)) n.set(a, c);
  ...
  for (const n of e.querySelectorAll("head link[rel=stylesheet]"))
    if (
      !document.querySelector(
        `[${M}="${n.getAttribute(M)}"], link[rel=stylesheet][href="${n.getAttribute("href")}"]`,
      )
    ) {
...
async function C(e, t, n, o, r) {
  async function i(d) {
    function l(f) {
      const p = f.effect;
      return !p || !(p instanceof KeyframeEffect) || !p.target
        ? !1
        : window.getComputedStyle(p.target, p.pseudoElement)
  ...
  const u = async () => {
    ...
    a = document.title,
    c = await ve(e, n.viewTransition, u);
  z(c.to, c.from, t, a, o),
    D(pe),
    r === "animate" &&
...
async function J(e, t, n, o, r) {
  ...
  async function c(s) {
    ...
    if (
      ((_ ??= new DOMParser()),
      (s.newDocument = _.parseFromString(f.html, f.mediaType)),
    ...
    const p = Se(s.newDocument);
    p.length && !s.signal.aborted && (await Promise.all(p));
  }
  async function d() {
  ...
  if ((document.documentElement.setAttribute(F, a.direction), x))
...
function Pe() {
  for (const e of ["touchstart", "mousedown"])
    document.addEventListener(
      e,
      (t) => {
        T(t.target, "tap") && R(t.target.href, { ignoreSlowConnection: !0 });
      },
      { passive: !0 },
    );
}
...
function Ie() {
  ...
  O(() => {
    for (const t of document.getElementsByTagName("a"))
      E.has(t) || (T(t, "viewport") && (E.add(t), (e ??= xe()), e.observe(t)));
  });
}
function xe() {
  const e = new WeakMap();
...
function R(e, t) {
  ...
  if (Oe(e, n))
    if (
      (Q.add(e), document.createElement("link").relList?.supports?.("prefetch"))
    ) {
      const o = document.createElement("link");
      (o.rel = "prefetch"), o.setAttribute("href", e), document.head.append(o);
    } else {
      const o = new Headers();
      for (const [r, i] of Object.entries(U)) o.set(r, i);
...
(x || He() !== "none") &&
  ...
  document.addEventListener("submit", (e) => {
    ...
    const n = e.submitter,
      o = n && n === A;
    if (((A = null), t.tagName !== "FORM" || e.defaultPrevented || q(t) || o))
      return;
    const r = t,
      i = new FormData(r, n),
    ...
    if (d === "dialog" || location.origin !== new URL(c, location.href).origin)
      return;
    const l = { sourceElement: n ?? r };
    if (d === "get") {
      const s = new URLSearchParams(i),
        h = new URL(c);
      (h.search = s.toString()), (c = h.toString());
    } else l.formData = i;
    e.preventDefault(), X(c, l);
  }),
  ke({ prefetchAll: !0 }));
