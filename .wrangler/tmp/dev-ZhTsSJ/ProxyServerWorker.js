...
// ../../../AppData/Local/pnpm-cache/dlx/be1e7ee7d404231aa8601a124a3e24ff620a6d44355f5582f73f002ad19ae1a8/19d0c3261b0-90d8/node_modules/.pnpm/wrangler@4.76.0/node_modules/wrangler/wrangler-dist/ProxyServerWorker.js
  ...
  deliverTo(parent, property, promises) {
    ...
    if (this.value instanceof RpcPromise) {
      ...
      for (let record of this.promises) {
        _RpcPayload.deliverRpcPromiseTo(
          record.promise,
          record.parent,
          record.property,
          promises,
...
var PayloadStubHook = class _PayloadStubHook extends ValueStubHook {
  ...
  dup() {
    ...
    return new _PayloadStubHook(
      RpcPayload.deepCopyFrom(thisPayload.value, void 0, thisPayload),
    );
  }
  pull() {
...
var Devaluator = class _Devaluator {
  ...
  devaluateImpl(value, parent, depth) {
    ...
    switch (kind) {
      ...
      case "request": {
        ...
        if (
          cfReq.encodeResponseBody &&
          cfReq.encodeResponseBody !== "automatic"
        ) {
  ...
  devaluateHook(type, hook) {
    ...
    let exportId =
      type === "promise"
        ? this.exporter.exportPromise(hook)
          ...
          case "remap": {
            ...
            let captures = value[3].map((cap) => {
              if (
                ...
                throw new TypeError(
                  `unknown map capture: ${JSON.stringify(cap)}`,
                );
              }
              if (cap[0] === "export") {
...
var ImportTableEntry = class {
  ...
  dispose() {
    if (this.resolution) {
      ...
      this.abort(
        new Error("RPC was canceled because the RpcPromise was disposed."),
      );
      this.sendRelease();
    }
  }
...
var RpcSessionImpl = class {
  ...
  releaseExport(exportId, refcount) {
    ...
    if (entry.refcount < refcount) {
      throw new Error(
        `refcount would go negative: ${entry.refcount} < ${refcount}`,
      );
    }
      ...
      let resolve = /* @__PURE__ */ __name(async () => {
        ...
        for (;;) {
          ...
          if (payload.value instanceof RpcStub) {
            let { hook: inner, pathIfPromise } = unwrapStubAndPath(
              payload.value,
            );
            if (pathIfPromise && pathIfPromise.length == 0) {
  ...
  getImport(hook) {
    if (
      hook instanceof RpcImportHook &&
      hook.entry &&
      hook.entry.session === this
    ) {
  ...
  getPipeReadable(exportId) {
    ...
    if (!entry || !entry.pipeReadable) {
      throw new Error(
        `Export ${exportId} is not a pipe or its readable end was already consumed.`,
      );
    }
          ...
          case "reject": {
            ...
            if (typeof importId == "number" && msg.length > 2) {
              ...
              if (imp) {
                if (msg[0] == "resolve") {
                  imp.resolve(
                    new PayloadStubHook(new Evaluator(this).evaluate(msg[2])),
                  );
                } else {
  ...
  constructor(transport, localMain, options = {}) {
    ...
    if (localMain) {
      ...
      mainHook = new ErrorStubHook(
        new Error("This connection has no main object."),
      );
    }
...
var MapBuilder = class {
  ...
  makeOutput(result) {
    ...
    if (this.context.parent) {
      ...
      return new MapVariableHook(
        this.context.parent,
        this.context.parent.instructions.length,
      );
    } else {
      return this.context.subject.map(
        this.context.path,
...
var MapApplicator = class {
  ...
  apply(instructions) {
    try {
      ...
      return new Evaluator(this).evaluateCopy(
...
function applyMapToElement(input, parent, owner, captures, instructions) {
  let inputHook = new PayloadStubHook(
    RpcPayload.deepCopyFrom(input, parent, owner),
  );
    ...
    if (input instanceof RpcPromise) {
      ...
      try {
        for (let elem of input) {
          payloads.push(
            applyMapToElement(elem, input, owner, captures, instructions),
          );
        }
      } catch (err) {
...
var WritableStreamStubHook = class _WritableStreamStubHook extends StubHook {
  ...
  call(path, args) {
    try {
      ...
      return new PromiseStubHook(
        promise.then((payload) => new PayloadStubHook(payload)),
      );
    } catch (err) {
      return new ErrorStubHook(err);
    }
  ...
  get(path) {
    return new ErrorStubHook(
      new Error("Cannot access properties on a WritableStream stub"),
    );
  }
  dup() {
    let state = this.getState();
...
var FlowController = class {
  ...
  onAck(token) {
    ...
    if (this.firstAckTime === 0) {
      ...
      let growthFactor = this.inStartupPhase
...
function createWritableStreamFromHook(hook) {
  ...
  return new WritableStream({
    ...
    abort(reason) {
      ...
      const { promise } = hook.stream(
        ["abort"],
        RpcPayload.fromAppParams([reason]),
      );
      promise.then(
        () => disposeHook(),
        () => disposeHook(),
...
var ReadableStreamStubHook = class _ReadableStreamStubHook extends StubHook {
  ...
  static create(stream) {
    return new _ReadableStreamStubHook({
      refcount: 1,
      stream,
      canceled: false,
    });
  ...
  call(path, args) {
    args.dispose();
    return new ErrorStubHook(
      new Error("Cannot call methods on a ReadableStream stub"),
    );
  }
  map(path, captures, instructions) {
    for (let cap of captures) {
  ...
  get(path) {
    return new ErrorStubHook(
      new Error("Cannot access properties on a ReadableStream stub"),
    );
  }
  dup() {
    let state = this.state;
...
// ../../../AppData/Local/pnpm-cache/dlx/be1e7ee7d404231aa8601a124a3e24ff620a6d44355f5582f73f002ad19ae1a8/19d0c3261b0-90d8/node_modules/.pnpm/wrangler@4.76.0/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// .wrangler/tmp/bundle-lQpwiZ/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
];
var middleware_insertion_facade_default = ProxyServerWorker_default;

// ../../../AppData/Local/pnpm-cache/dlx/be1e7ee7d404231aa8601a124a3e24ff620a6d44355f5582f73f002ad19ae1a8/19d0c3261b0-90d8/node_modules/.pnpm/wrangler@4.76.0/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-lQpwiZ/middleware-loader.entry.ts
