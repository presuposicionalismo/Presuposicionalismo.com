<script lang="ts">
  ...
  let activeCategory = $state<"tags" | "authors">("tags");
    ...
    >
      ...
      <Command.Root class="flex flex-col">
        <div class="flex items-center gap-2.5 border-b border-border px-4">
          ...
          <Command.Input
            placeholder={activeCategory === "tags"
              ? "Buscar etiqueta..."
              : "Buscar autor..."}
        ...
        <div class="flex border-b border-border px-2" role="tablist">
          <button
            type="button"
            role="tab"
            aria-selected={activeCategory === "tags"}
            onclick={() => (activeCategory = "tags")}
            class="border-b-2 px-3 py-2.5 text-sm font-medium transition-colors {activeCategory ===
            'tags'
          ...
          {#if authors.length > 0}
            <button
              type="button"
              role="tab"
              aria-selected={activeCategory === "authors"}
              onclick={() => (activeCategory = "authors")}
              class="border-b-2 px-3 py-2.5 text-sm font-medium transition-colors {activeCategory ===
              'authors'
                ? 'border-accent text-accent'
                : 'border-transparent text-muted hover:text-foreground'}"
